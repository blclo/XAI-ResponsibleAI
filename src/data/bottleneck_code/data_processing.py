...
def extract_data(data_dir):
    ...
    cwd = os.getcwd()
    data_path = Path(join(cwd,data_dir + '/images')).as_posix()
    val_ratio = 0.2
    ...
    with open(data_path.replace('images', 'images.txt'), 'r') as f:
        for line in f:
            ...
            path_to_id_map[Path(join(data_path, items[1])).as_posix()] = int(items[0])
    ...
    for i, folder in enumerate(folder_list):
        ...
        for cf in classfile_list:
            img_id = path_to_id_map[Path(join(folder_path, cf)).as_posix()]
            img_path = Path(join(folder_path, cf)).as_posix()
...
if __name__ == "__main__":
    ...
    for dataset in ['train','val','test']:
        ...
        os.makedirs(Path(args.save_dir).as_posix(), exist_ok=True)
        f = open((Path(args.save_dir) / (dataset + '.pkl')).as_posix(), 'wb')
        if 'train' in dataset:
